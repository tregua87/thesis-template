...
In Section~\ref{sec:model-examples_sgxmonitor}, we describe the usage of our 
model for Intel SGX SDK, in particular, we model the \emph{outside function} 
invocation (Section~\ref{ssec:ocall-example}) and the exception handling 
(Section~\ref{ssec:exception-handling}).



\section{Case Study: Intel SGX SDK}
...
\begin{figure}[t]
	...
	\end{subfigure}
	\caption[SGX \emph{exception handling} modeling.]{Example of 
	\emph{exception 
			handling} modeling. We show the FSM representation and the 
			transaction 
