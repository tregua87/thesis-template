...
TEE technologies achieve a strong security level mainly thanks to sound
\emph{memory isolation} and \emph{remote attestation}.
Unfortunately, these properties also introduce new challenges that range from 
scalability to new type of intrusion.
Moreover, the current integrity mechanisms mainly focus on static properties 
(\ie if a piece of code is intact) while advanced solutions only fit embedded 
systems scenarios.
...
The \emph{memory isolation} is enforced at hardware level with new CPU opcodes 
and extra checks at microcode level, thus having an OS independent design.
