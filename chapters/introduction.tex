...
Unfortunately, these properties also introduce new challenges. For instance, 
the total amount of isolated memory is often limited, thus arising 
scalability issues. Or else, an adversary might mount new type of intrusions 
due the memory isolation.
If this happens, detecting threats hidden in trusted regions becomes crucial.
In addition, the current integrity mechanisms mainly focus on static properties 
(\ie if a piece of code is intact) while overlooking runtime attacks, and the 
few advanced solutions existing simply focus on embedded systems.
In this section, we dig into the main issues that afflict \emph{memory 
...
For instance, we have to handle the interaction between the trusted region and 
the external word (\eg to write a file), or else, these un-observable portions 
of memory may help an adversary to hide its presence in the machine.
...
some relies on a policy-based verification schema~\citep{aberadiat}; 
...
and then further elaborate in dedicated chapters.
