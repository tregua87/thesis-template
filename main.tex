...
Are you sure the last Virtual Machine brought from your Could Provider is 
working properly?
The widespread adoption of cloud infrastructures allowed online business 
to benefit of larger market segments, parallelly, armies of adversaries enjoyed 
a vaster set of possible victims.
In addition, leaving part of the infrastructure on cloud providers de facto 
...
Finally, leading companies and customers to the need of advanced protections 
for their products and data.
...
\emph{Trusted regions} provide two main properties: \emph{memory isolation} and 
\emph{remote attestation}. The former ensures physical isolation of 
\emph{trusted regions}, thus avoiding direct writing to, and reading from, the 
...
We address scalability issues affecting TEEs and propose more 
flexible designs fitting the current hardware specifications 
...
Finally, we study the implication of \emph{trusted regions} when performing 
incident response, propose new techniques, and show research directions to 
fulfill the current gaps (Chapter~\ref{chp:forensic}).
...
Then, we propose another model that can be included inside a \emph{trusted 
region}, thus stretching runtime protections over \emph{memory isolated} 
...
further research questions regarding TEE in cloud computing.
